...
def xyz_to_nuclei_fod(ase_atoms):
    ...
    for i in range(0, syssize):
        ...
        if ase_atoms[i].symbol == 'X' or ase_atoms[i].symbol == 'He':
            distold = fodeps
            for j in range(0, i):
                dist = np.sqrt((ase_atoms[i].position[0]-ase_atoms[j].position[0])**2+(
                    ase_atoms[i].position[1]-ase_atoms[j].position[1])**2+(ase_atoms[i].position[2]-ase_atoms[j].position[2])**2)
    ...
    for i in range(0, syssize):
        if i < nrofnuclei:
            tmp.append(ase_atoms[i].symbol+' '+str(ase_atoms[i].position[0])+' '+str(
                ase_atoms[i].position[1])+' '+str(ase_atoms[i].position[2])+'\n')
            nuclei.append(ase_atoms[i])
        elif ase_atoms[i].symbol == 'X':
            fod1.append(ase_atoms[i])
            if included[i] == 1:
                tmp.append('ghost1'+' '+str(ase_atoms[i].position[0])+' '+str(
                    ase_atoms[i].position[1])+' '+str(ase_atoms[i].position[2])+'\n')
        elif ase_atoms[i].symbol == 'He':
            fod2.append(ase_atoms[i])
            if included[i] == 1:
                tmp.append('ghost2'+' '+str(ase_atoms[i].position[0])+' '+str(
                    ase_atoms[i].position[1])+' '+str(ase_atoms[i].position[2])+'\n')
    geo = ''.join(tmp)
...
def xyz2cfg(p_xyz, er):
    ...
    for n in range(len(nuc_sym)):
        o.write(str(nuc_pos[n][0])+' '+str(nuc_pos[n][1])+' ' +
                str(nuc_pos[n][2])+' '+str(atomic_numbers[nuc_sym[n]])+'\n')
    o.write('@electrons\n')
    ...
    for e in range(len(fod1_sym)):
        ...
        o.write(str(fod1_pos[e][0])+' '+str(fod1_pos[n][1]) +
                ' '+str(fod1_pos[n][2])+' 1 '+str(er)+'\n')
    ...
    for e in range(len(fod2_sym)):
        ...
        o.write(str(fod2_pos[e][0])+' '+str(fod2_pos[n][1]) +
                ' '+str(fod2_pos[n][2])+' -1 '+str(er)+'\n')
    o.close()
