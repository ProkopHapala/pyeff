...
def sympy_E_rspace(value_r, value_qi, value_qj, value_alphai, value_alphaj, value_alphamax, debug=None):
    ...
    #
    Erspace = qi*qj*(1/r*erf((sqrt((alphai*alphaj)/(alphai+alphaj))*r)) -
                     1/r*erf((sqrt((alphai*alphamax)/(alphai+alphamax))*r)))
    ...
    #
    Erspace = lambdify([r, qi, qj, alphai, alphaj, alphamax],
                       Erspace, modules=['numpy', 'sympy'])
    dErspacedr = lambdify([r, qi, qj, alphai, alphaj, alphamax],
                          dErspacedr, modules=['numpy', 'sympy'])
    dErspacedalphai = lambdify(
        [r, qi, qj, alphai, alphaj, alphamax], dErspacedalphai, modules=['numpy', 'sympy'])
    dErspacedalphaj = lambdify(
        [r, qi, qj, alphai, alphaj, alphamax], dErspacedalphaj, modules=['numpy', 'sympy'])
    ...
    ana_Erspace = Erspace(value_r, value_qi, value_qj,
                          value_alphai, value_alphaj, value_alphamax)
    ana_dErspacedr = dErspacedr(
        value_r, value_qi, value_qj, value_alphai, value_alphaj, value_alphamax)
    num_Erspace = fun(value_r, value_qi, value_qj,
                      value_alphai, value_alphaj, value_alphamax)
    num_dErspacedr = -1 * \
        dfundr(value_r, value_qi, value_qj, value_alphai,
               value_alphaj, value_alphamax)
...
def sympy_E_self(value_qi, value_alphai, debug=None):
    ...
    #
    Eself1 = -1/2.*qi**2.*(2./sqrt(pi)) * \
        (sqrt((alphai*alphamax)/(alphai+alphamax)))
