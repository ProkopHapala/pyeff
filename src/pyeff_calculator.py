...
class pyeff(FileIOCalculator):
    ...
    def __init__(self, restart=None, ignore_bad_restart_file=False,
                 label=os.curdir,  factor=1., atoms=None, **kwargs):
    ...
    def calculate(self, atoms, properties=['energy'], system_changes=['positions']):
        ...
        # pyeff_energy_force calculation
        [pyeff_calc, Z, R, r, s, delta] = read_cfg(p_cfg='pyeff.cfg', factor=1./self.factor )
    ...
    def write_input(self, atoms):
        ...
        for p in range(len(self.types)):
            if self.types[p] == 'nuclei':
                o.write(str(positions[p][0])+' '+str(positions[p][1])+' ' +
                        str(positions[p][2])+' '+str(self.chemical_symbols[p])+'\n')
        o.write('@electrons\n')
        for p in range(len(self.types)):
            if self.types[p] == 'electron':
                o.write(str(positions[p][0])+' '+str(positions[p][1])+' '+str(
                    positions[p][2])+' '+str(self.spins[p])+' '+str(sizes[p])+'\n')
        o.close()
    ...
    def pos_transform(self, atoms):
        ...
        for p in range(len(self.types)):
            if self.types[p] == 'nuclei':
                ...
                forces.append(
                    [self.forces[idx+0], self.forces[idx+1], self.forces[idx+2]])
                sizes.append(0)
                fr.append(0)
                idx = idx + 3
            if self.types[p] == 'electron':
    ...
    def printAtoms(self, factor=const_bohr ):
        for i in range(len(self.aZs)):
            print( "atom[%i] Z %i pos(%g,%g,%g) " %(i, self.aZs[i], self.apos[i][0]*factor,self.apos[i][1]*factor,self.apos[i][2]*factor) )

    def printElectrons(self, factor=const_bohr):
        for i in range(len(self.aZs)):
            print( "electron[%i] pos(%g,%g,%g) size %g spin %i" %(i,  self.epos[i][0]*factor,self.epos[i][1]*factor,self.epos[i][2]*factor, self.esize[i]*factor, self.espin[i]) )
        #self.aZs   = Z
        #self.apos  = R
        #self.epos  = r
        #self.esize = s
