...
def read_cfg_pbc(p_cfg, print_data=None, scale=1.0):
    #
    ...
    for l in range(len(ll)):
        ...
        if ll[l].find(tag_Lx) != -1:
            Lx = float(ll[l].split()[-1])*scale
        if ll[l].find(tag_Ly) != -1:
            Ly = float(ll[l].split()[-1])*scale
        if ll[l].find(tag_Lz) != -1:
            Lz = float(ll[l].split()[-1])*scale
    Z = []
    R = []
    r = []
    s = []
    ...
    for n in range(idx_nuc_pos_start, idx_elec_pos_start-1):
        ...
        tmp = ll[n].split()
        R.append([float(tmp[0])*scale, float(tmp[1])
                 * scale, float(tmp[2])*scale])
    ...
    for i in range(es.numcharges):
        ...
        for j in range(es.numcharges):
            if (es.ewald[j].palpha > es.max_alpha):
                a1 = np.sqrt(
                    es.ewald[i].palpha*es.ewald[j].palpha/(es.ewald[i].palpha+es.ewald[j].palpha))
                a2 = np.sqrt(es.ewald[i].palpha*es.max_alpha /
                             (es.ewald[i].palpha+es.max_alpha))
                for gp in range(es.num_gps):
                    if i != j or gp != 0:
                        ...
                        if (r < es.max_r):
                            ...
                            dE_rspacedr = es.ewald[i].q * \
                                es.ewald[j].q*(dE1+dE2+dE3)
                            ...
                            dE1dalphai = -1*es.ewald[j].palpha*a1*np.exp(-1*(a1**2.)*(r**2.))/(
                                np.sqrt(np.pi)*es.ewald[i].palpha*(es.ewald[i].palpha + es.ewald[j].palpha))
                            dE2dalphai = es.max_alpha*a2*np.exp(-1*(a2**2)*(r**2))/(
                                np.sqrt(np.pi)*es.ewald[i].palpha*(es.ewald[i].palpha + es.max_alpha))
                            falpha_i = -1 * \
                                es.ewald[i].q*es.ewald[j].q * \
                                (dE1dalphai + dE2dalphai)
                            ...
                            dE1dalphaj = -es.ewald[i].palpha*a1*np.exp(-1.*(a1**2.)*(r**2))/(
                                np.sqrt(np.pi)*es.ewald[j].palpha*(es.ewald[i].palpha + es.ewald[j].palpha))
                            dE2dalphaj = 0
                            falpha_j = -1 * es.ewald[i].q * \
                                es.ewald[j].q*(dE1dalphaj)

                            # update
                            es.ewald[i].update(px=0, py=0, pz=0, palpha=0, q=0, energy=0.5 *
                                               energy, fx=-0.5*fx, fy=-0.5*fy, fz=-0.5*fz, falpha=-0.5*falpha_i)
                            es.ewald[j].update(px=0, py=0, pz=0, palpha=0, q=0, energy=0, fx=(
                                -0.5)*-1*fx, fy=(-0.5)*-1*fy, fz=(-0.5)*-1*fz, falpha=(-0.5)*falpha_j)
...
def E_kspace(es):
    ...
    for i in range(es.num_kps):
        ...
        rho_k_cap_imag = 0
        for j in range(es.numcharges):
            angle = -1.*(es.kgrid_x[i]*es.ewald[j].px+es.kgrid_y[i]
                         * es.ewald[j].py+es.kgrid_z[i]*es.ewald[j].pz)
            # print angle
            mag = es.ewald[j].q * \
                np.exp(-1*es.kgrid_k2[i]/(4.*es.ewald[j].palpha))
            ewald_S_real.append(mag*np.cos(angle))
            ewald_S_imag.append(mag*np.sin(angle))
            if es.ewald[j].palpha > es.max_alpha:
                mag_cap = es.ewald[j].q * \
                    np.exp(-1.*es.kgrid_k2[i]/(4.*es.max_alpha))
        ...
        j = 0
        for j in range(es.numcharges):
            E_kspacedr = ewald_S_real[j]*rho_k_cap_imag-ewald_S_imag[j]*rho_k_cap_real + \
                ewald_S_cap_real[j]*rho_k_imag-ewald_S_cap_imag[j]*rho_k_real
            E_kspacedr = E_kspacedr/es.kgrid_k2[i]
            fx = E_kspacedr*es.kgrid_x[i]
            fy = E_kspacedr*es.kgrid_y[i]
            fz = E_kspacedr*es.kgrid_z[i]
            if es.ewald[j].palpha > es.max_alpha:
                falpha = (ewald_S_real[j]*rho_k_cap_real+ewald_S_imag[j]
                          * rho_k_cap_imag)/(es.ewald[j].palpha**2.)

            else:
                falpha = (ewald_S_real[j]*rho_k_cap_real+ewald_S_imag[j]*rho_k_cap_imag +
            ...
            es.ewald[j].update(px=0, py=0, pz=0, palpha=0, q=0,
                               energy=0, fx=fx, fy=fy, fz=fz, falpha=falpha)

        ewald_i = (1.0 / es.kgrid_k2[i]) * (rho_k_real *
                                            rho_k_cap_real + rho_k_imag * rho_k_cap_imag)
        ewald_energy = ewald_energy + ewald_i
        # print('%0.7f' % ewald_energy)
    ewald_energy = ewald_energy * \
        (2*np.pi/V)/float(es.numcharges)  # /es.num_kps
    # print('%0.7f' % ewald_energy)
    i = 0
    # update energy
    for i in range(es.numcharges):
        es.ewald[i].update(px=0, py=0, pz=0, palpha=0, q=0,
                           energy=ewald_energy, fx=0, fy=0, fz=0, falpha=0)
    return ewald_energy
...
def E_self(es):
    ...
    for i in range(es.numcharges):
        if es.ewald[i].palpha >= es.max_alpha:
            ...
            dEselfdalphai = 0.5*es.max_alpha*(es.ewald[i].q**2.)*a2/(
                np.sqrt(np.pi)*es.ewald[i].palpha*(es.ewald[i].palpha + es.max_alpha))
            # eff code
            ...
            Eself = -0.5 * \
                np.sqrt(2)*np.sqrt(es.ewald[i].palpha) * \
                (es.ewald[i].q**2.)/np.sqrt(np.pi)
            dEselfdalphai = 0.25 * \
                np.sqrt(2)*(es.ewald[i].q**2.) / \
                (np.sqrt(np.pi)*np.sqrt(es.ewald[i].palpha))
            # eff code
            # a1 = np.sqrt(es.ewald[i].palpha/2.)
            # dEselfdalphai = (es.ewald[i].q**2.) / (4. * a1) / np.sqrt(np.pi)
        es.ewald[i].update(px=0, py=0, pz=0, palpha=0, q=0,
                           energy=Eself, fx=0, fy=0, fz=0, falpha=dEselfdalphai)
...
def E_uniform(es):
    ...
    for i in range(es.numcharges):
        if es.ewald[i].palpha >= es.max_alpha:
            E_uniform = fac * es.ewald[i].q * \
                (1/es.max_alpha - 1/es.ewald[i].palpha)
            dE_uniformdalphai = -1*fac * \
                es.ewald[i].q / (es.ewald[i].palpha**2.)
        else:
            E_uniform = 0
            dE_uniformdalphai = 0
        E_uniform_sum = E_uniform_sum + E_uniform
        es.ewald[i].update(px=0, py=0, pz=0, palpha=0, q=0,
                           energy=E_uniform, fx=0, fy=0, fz=0, falpha=dE_uniformdalphai)
...
def E_PauliPeriodic(es):
    ...
    def E_up_down(rho, r, si, sj):
        ...
        E_up_down = -8.0*rho*(-(-4.0*r**2 + 6.0*si**2 + 6.0*sj**2)/(si**2 + sj**2)**2 + 1.5/sj**2 + 1.5/si**2)*(1/(si/sj + sj/si)
                                                                                                                )**3.0*np.exp(-2.0*r**2/(si**2 + sj**2))/(8.0*(1/(si/sj + sj/si))**3.0*np.exp(-2.0*r**2/(si**2 + sj**2)) + 1.0)
    ...
    for i in range(len(es.system.elec)):
        ...
        for j in range(0, i):
            ...
            if spin_i != spin_j:
                E2, E2dr, E2ds1, dE2ds2 = E_up_down(
                    Pauli_rho, Pauli_r*x_ij, Pauli_s*s_i, Pauli_s*s_j)
                # print E2
                fr = E2dr
                fs1 = E2ds1
