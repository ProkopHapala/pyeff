...
class pyeff_pbc(FileIOCalculator):
    ...
    def __init__(self, restart=None, ignore_bad_restart_file=False,
        ...
        for arg, val in list(self.parameters.items()):
    ...
    def initialize(self, atoms=None, properties=['energy'], system_changes=['positions']):
        ...
        [Lx, Ly, Lz, pyeff_calc, Z, R, r, s, delta] = read_cfg_pbc(
            self.p_cfg, scale=self.scale)
        # Get box lengths
        self.Lx = Lx
        self.Ly = Ly
        self.Lz = Lz
        ...
        for n in range(len(ewald_calc.system.nuc)):
            ewald_calc.system.nuc[n].update(px=0, py=0, pz=0, pr=0, spin=0, energy=-1*ewald_calc.system.nuc[n].energy, fx=-1*ewald_calc.system.nuc[n].fx,
                                            fy=-1*ewald_calc.system.nuc[n].fy, fz=-1*ewald_calc.system.nuc[n].fz, fr=-1*ewald_calc.system.nuc[n].fr)
        for e in range(len(ewald_calc.system.elec)):
            ewald_calc.system.elec[e].update(px=0, py=0, pz=0, pr=0, spin=0, energy=-1*ewald_calc.system.elec[e].energy, fx=-1*ewald_calc.system.elec[e].fx,
                                             fy=-1*ewald_calc.system.elec[e].fy, fz=-1*ewald_calc.system.elec[e].fz, fr=-1*ewald_calc.system.elec[e].fr)
        ...
        atoms = Atoms(''.join(self.get_chemical_symbols()), positions=np.array(
            ewald_calc.system.positions()))  # *bohr)
        self.atoms = atoms
    ...
    def pos_transform(self, atoms):
        ...
        for p in range(len(self.types)):
            if self.types[p] == 'nuclei':
                positions.append(
                    [self.positions1d[idx+0], self.positions1d[idx+1], self.positions1d[idx+2]])
                forces.append(
                    [self.forces1d[idx+0], self.forces1d[idx+1], self.forces1d[idx+2]])
                sizes.append(0)
                fr.append(0)
                idx = idx + 3
            if self.types[p] == 'electron':
    ...
    def write_frmorb(self):
        ...
        for p in range(len(self.types)):
            if self.types[p] == 'electron':
                o.write(
                    str(positions[p][0])+' '+str(positions[p][1])+' '+str(positions[p][2])+'\n')
        o.close()
