...
def read_cfg(p_cfg, print_data=None, factor=1. ):
    #
    ...
    for n in range(len(R)):
        calc.nuc[n].px = R[n][0]*factor
        calc.nuc[n].py = R[n][1]*factor
        calc.nuc[n].pz = R[n][2]*factor
    for e in range(len(r)):
        calc.elec[e].px = r[e][0]*factor
        calc.elec[e].py = r[e][1]*factor
        calc.elec[e].pz = r[e][2]*factor
        calc.elec[e].pr = s[e]   *factor
...
def E_nuc_elec(calc, Z, R, r, s, debug=None):
    ...
    for i in range(len(Z)):
        for j in range(len(r)):
            ...
            fr = 2.*np.sqrt(2.)*Z[i]*np.exp(-2.*R_ij**2/s[j]**2.) / \
                (np.sqrt(np.pi)*R_ij*s[j])-Z[i] * \
                erf(np.sqrt(2.)*R_ij/s[j])/R_ij**2
            fr = -1*fr
            if debug != None:
            ...
            # from sympy calc
            # dE/ds
            fs = -2*np.sqrt(2)*Z[i]*np.exp(-2*R_ij**2/s[j]
                                           ** 2)/(np.sqrt(np.pi)*s[j]**2)
            # update the calculations object (calc)
            calc.nuc[i].update(energy=e/2., px=0, py=0, pz=0,pr=0, spin=0, fx=-1*fx, fy=-1*fy, fz=-1*fz, fr=0)
    ...
    for i in range(len(r)):
        for j in range(len(r)):
            if j < i:
                ...
                # from sympy calc
                # dE/dR
                fr = (erf(np.sqrt(2)*x_ij/np.sqrt(s[i]**2 + s[j]**2))/x_ij - 2.*np.sqrt(
                    2)*np.exp(-2.*x_ij**2/(s[i]**2 + s[j]**2))/(np.sqrt(np.pi)*np.sqrt(s[i]**2 + s[j]**2)))/x_ij
                if debug != None:
                ...
                calc.elec[i].update(energy=e/2., px=0, py=0, pz=0,
...
def E_Pauli(calc, delta, s, r, debug=None, parameters=None):
    ...
    def Delta_T_ij(r, si, sj):
        # energy
        Delta_T_ij = (3./2.)*((1./(si**2))+(1./(sj**2)))-2. * \
            (3.*((si**2)+(sj**2))-2.*(r**2))/(((si**2)+(sj**2))**2)
        return Delta_T_ij
